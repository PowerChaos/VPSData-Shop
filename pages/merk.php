...
if (isset($_GET['merk'])) {
	$id = $_GET['merk'];
	try {
		$stmt = $db->prepare("SELECT * FROM products WHERE merk = :id GROUP BY (cat)ORDER BY cat");
...
<div class="welcome">
	 <div class="container">
		 ...
		 <div class="col-md-9 welcome-right">
			 <h3>Hier vind u alle producten terug van <?php echo $id ?></h3>
		 </div>
	 </div>
</div>
...
<?php
foreach ($row as $cat){
			$stmt2 = $db->prepare("SELECT * FROM products WHERE cat = :cat AND merk = :merk ORDER BY RAND()");
			$stmt2->execute(array(':merk' => $cat[merk],':cat' => $cat[cat]));
		...
		$totstar = ($ratingcount > '0')?($starcount / $ratingcount):"0";
		 ?>
					 <a href="//<?php echo $_SERVER['SERVER_NAME'].'/'.$seomerk.'/'.$seoproduct;?>.html"><div class="product-grid love-grid">
						...
						<img src="<?php echo ($img[img]=="")?$defimg:$img[img] ?>" height="280" alt="<?php echo $prod[name] ?>"/>
						</div>					
						<div class="product-info">
							<div class="product-info-cust prt_name">
								<h4><?php echo $prod['name'] ?></h4>
								<input id="stars" name="stars" class="rating rating-loading" value="<?php echo round($totstar)?>" data-min="0" data-max="5" data-step="1" data-size="md" data-show-clear="false" data-readonly="true" data-show-caption="false"><p>( <?php echo $ratingcount ?> Stemmen )</p></input>							
							</div>													
						</div>
					</div></a>
		 <?php 
