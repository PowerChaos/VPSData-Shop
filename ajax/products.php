...
{	
	case "stock":
		...
		<button type="button" class="btn-sm btn-primary text-center" data-toggle="modal" data-target="#modal" id="<?php echo $waarde ?>" onclick="extra(this.id,'stock');"><i class="material-icons" title="kenmerk Toevoegen" aria-hidden="true">person_add</i><span class="sr-only">kenmerk Toevoegen</span></button>
		<br><br>
		...
		if (confirm("Zet op -1 als het product NIET meer beschikbaar is\nklik op OK om Dit kenmerk te verwijderen")){;
			$.ajax({
				type: "POST",
				url: "../ajax/remove.php",
